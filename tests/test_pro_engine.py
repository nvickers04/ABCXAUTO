...
def test_a_good_look_re_arms_paper_stay_up():
    """Paper RTH / premarket keep looking. No sit clock."""
...
def test_a_good_look_does_not_rearm_closed():
    eng = ProEngine()
...
@pytest.mark.asyncio
async def test_host_think_resume_is_optional_on_older_grok_turn_mocks(monkeypatch):
    """stay-up resume= must not blow mocks that omit the kwarg."""
    from abcxauto.brain import BrainTurn
    ...
    assert out.get("_failed") is False
    assert "looking" in str(out.get("rationale") or "")
...
async def test_finished_rth_look_does_not_write_a_sit_clock(monkeypatch, tmp_path):
    """Done criterion: a finished RTH look must not write grok_wake.json."""
    monkeypatch.setenv("ABCXAUTO_GROK_WAKE_PATH", str(tmp_path / "wake.json"))
    calls = {"n": 0}
...
@pytest.mark.asyncio
async def test_launch_ignores_a_leftover_rth_sit_clock(monkeypatch, tmp_path):
    """RTH leftover grok_wake.json must not block Start."""
    monkeypatch.setenv("ABCXAUTO_GROK_WAKE_PATH", str(tmp_path / "wake.json"))
...
@pytest.mark.asyncio
async def test_rth_parked_flag_stays_up_without_a_sit_clock(monkeypatch, tmp_path):
    """A _parked look in RTH is not overnight park. Stay-up continues."""
    monkeypatch.setenv("ABCXAUTO_GROK_WAKE_PATH", str(tmp_path / "wake.json"))
    calls = {"n": 0}
...
@pytest.mark.asyncio
async def test_paper_premarket_stays_up_without_a_sit_clock(monkeypatch, tmp_path):
    monkeypatch.setenv("ABCXAUTO_GROK_WAKE_PATH", str(tmp_path / "wake.json"))
    calls = {"n": 0}
