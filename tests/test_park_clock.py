...
def test_ensure_park_overnight_silent_seeds(tmp_path, monkeypatch):
    from abcxauto.park_clock import ensure_park, load_alarm

    monkeypatch.setenv("ABCXAUTO_GROK_WAKE_PATH", str(tmp_path / "wake.json"))
    monkeypatch.setenv("ABCXAUTO_DEFAULT_LOOK_S", "30")
    alarm = ensure_park(previous_set_at="", session="closed")
    assert alarm.wake_at
    assert load_alarm().wake_at == alarm.wake_at


def test_ensure_park_rth_writes_no_sit_clock(tmp_path, monkeypatch):
    """A finished RTH look must not leave grok_wake.json. Clerk is not a runner."""
    from abcxauto.park_clock import ensure_park, load_alarm
...
def test_ensure_park_keeps_overnight_alarm(tmp_path, monkeypatch):
    from abcxauto.park_clock import ensure_park, load_alarm, set_wake

    monkeypatch.setenv("ABCXAUTO_GROK_WAKE_PATH", str(tmp_path / "wake.json"))
    set_wake(wake_in_s=120, wake_if=["fill"], session="closed")
    before = load_alarm()
    out = ensure_park(previous_set_at="", session="closed")
...
def test_paper_rth_set_wake_writes_no_sit_clock(tmp_path, monkeypatch):
    """RTH has no sit clock. set_wake in regular is a no-op."""
...
def test_set_wake_honors_a_long_overnight_park(tmp_path, monkeypatch):
    from abcxauto.park_clock import _parse_iso, _utc_now, set_wake
...
def test_clerk_look_s_rth_is_not_a_sit_clock(monkeypatch):
    """RTH clerk_look_s is unused as a launcher. Overnight still parks."""
    from abcxauto.park_clock import DEFAULT_LOOK_S, clerk_look_s
    ...
    assert clerk_look_s(flat=False, session="regular") == DEFAULT_LOOK_S
    assert clerk_look_s(flat=True, session="regular", next_look_s=None) == DEFAULT_LOOK_S
    assert clerk_look_s(flat=False, session="regular", next_look_s=120) == DEFAULT_LOOK_S
...
def test_clerk_look_s_last_hour_to_open_is_overnight_only(monkeypatch):
    from abcxauto.park_clock import LAST_HOUR_LOOK_S, DEFAULT_LOOK_S, clerk_look_s

    monkeypatch.delenv("ABCXAUTO_DEFAULT_LOOK_S", raising=False)
    # Premarket stay-up: not a park duration.
    assert clerk_look_s(
        flat=True, session="premarket", minutes_to_open=45
    ) == DEFAULT_LOOK_S
    assert clerk_look_s(
        flat=True, session="closed", minutes_to_open=45
    ) == LAST_HOUR_LOOK_S


def test_clerk_look_s_session_card_does_not_park_premarket(monkeypatch):
    """Gap cards cannot sit the think loop until 9:30. Premarket has no sit clock."""
    from abcxauto.park_clock import DEFAULT_LOOK_S, clerk_look_s
...
def test_ensure_park_clears_a_premarket_leftover(tmp_path, monkeypatch):
    from datetime import datetime, timedelta, timezone

    from abcxauto.park_clock import GrokAlarm, ensure_park, save_alarm
...
def test_ensure_park_clears_a_remaining_to_bell_leftover(tmp_path, monkeypatch):
    from datetime import datetime, timedelta, timezone
...
def test_remaining_to_bell_and_start_looks_now(tmp_path, monkeypatch):
    ...
    save_alarm(GrokAlarm(wake_at=soon, set_at=soon))
    assert start_looks_now(session="closed", minutes_to_open=6 * 60) is False
    assert start_looks_now(session="premarket", minutes_to_open=32) is True
    bell = (datetime.now(timezone.utc) + timedelta(minutes=32)).isoformat()
    save_alarm(GrokAlarm(wake_at=bell, set_at=bell))
    assert start_looks_now(session="closed", minutes_to_open=32) is True
    assert start_looks_now(session="closed", minutes_to_open=6 * 60) is False
    assert start_looks_now(session="regular") is True
...
def test_begin_run_overnight_parks(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "abcxauto.park_clock.et_minutes_to_rth_open", lambda **_k: 6 * 60.0
    )
    ts._run = {}
    ts.begin_run()
...
def test_ensure_park_spent_overnight_alarm_is_replaced(tmp_path, monkeypatch):
    from datetime import datetime, timedelta, timezone

    from abcxauto.park_clock import ensure_park, load_alarm, save_alarm, GrokAlarm

    monkeypatch.setenv("ABCXAUTO_GROK_WAKE_PATH", str(tmp_path / "wake.json"))
    monkeypatch.setenv("ABCXAUTO_DEFAULT_LOOK_S", "30")
    past = (datetime.now(timezone.utc) - timedelta(seconds=5)).isoformat()
    save_alarm(GrokAlarm(wake_at=past, set_at=past))
    alarm = ensure_park(session="closed", flat=True)
    assert alarm.wake_at
    assert alarm.wake_at != past
    assert load_alarm().wake_at == alarm.wake_at


def test_ensure_park_rth_clears_a_standing_clock(tmp_path, monkeypatch):
    from datetime import datetime, timedelta, timezone
    ...
    soon = (datetime.now(timezone.utc) + timedelta(seconds=60)).isoformat()
    save_alarm(GrokAlarm(wake_at=soon, set_at=soon))
    cleared = ensure_park(session="regular", flat=True)
    assert cleared.wake_at is None
    replaced = ensure_park(session="regular", flat=True, replace=True)
    assert replaced.wake_at is None


def test_paper_stay_up_sessions(monkeypatch):
    from abcxauto.park_clock import paper_stay_up, session_parks
    ...
    assert session_parks("closed") is True
    assert session_parks("postmarket") is True
    assert session_parks("regular") is False
    assert session_parks("premarket") is False
    monkeypatch.setattr(
        "abcxauto.config.Config.is_paper",
...
def test_no_wake_bus_module_left():
    import importlib.util

    assert importlib.util.find_spec("abcxauto.wake_bus") is None
    import abcxauto.park_clock as park_clock

    assert not hasattr(park_clock, "ensure_next_look")


def test_et_session_now_splits_the_day():
    from datetime import datetime
    from zoneinfo import ZoneInfo

    from abcxauto.park_clock import et_session_now

    et = ZoneInfo("America/New_York")
    assert et_session_now(now=datetime(2026, 8, 26, 3, 0, tzinfo=et)) == "closed"
    assert et_session_now(now=datetime(2026, 8, 26, 8, 32, tzinfo=et)) == "premarket"
    assert et_session_now(now=datetime(2026, 8, 26, 10, 0, tzinfo=et)) == "regular"
    assert et_session_now(now=datetime(2026, 8, 26, 17, 0, tzinfo=et)) == "postmarket"
    assert et_session_now(now=datetime(2026, 8, 22, 10, 0, tzinfo=et)) == "closed"
