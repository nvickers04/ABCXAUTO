...
def get_config() -> Config:
    ...
    session maps fall back to shared). Unknown ``model_params`` keys are
    refused at Settings; env / disk drop them with an error log.
    ``scan_fetch_cap`` from ``self_tune`` beats both.
    """
