...
class ProEngine:
    ...
    def start(self) -> str | None:
        ...
        try:
            ...
            if alarm.wake_at and not alarm.due() and not start_looks_now(alarm):
                # Overnight / after-close park stands. RTH leftover sit clocks
                # do not — Start looks now. Operator Start on a live worker
                # still pokes (already=True returned above).
                self._resume_think = False
    ...
    def _rearm_after_think(self, out: dict | None, *, session: str) -> float:
        """Stay-up re-arm or failed-look backoff. Return seconds to wait.

        Paper RTH / premarket keep looking on this process — empty / ``?``
        backs off 20–45s. A good look re-arms immediately (no sit clock).
        Overnight park is the file clock, not this path. Live does not re-arm.
        """
        ...
        if payload.get("_parked"):
            self._fail_streak = 0
            if paper_stay_up(session=session):
                self._resume_think = True
            return 0.0
    ...
    async def _host_think(
        ...
        from abcxauto.brain import _send_succeeded, grok_turn, invoke_grok_turn
        ...
        for item in getattr(turn, "sends", None) or []:
            if _send_succeeded((item or {}).get("result")):
                sends_ok += 1
        return {
            "cycle": n,
            "pnl": pnl,
    ...
    async def _async_loop(self, gen: int) -> None:
        ...
        try:
            while gen == self._gen and not self.stop.is_set():
                ...
                last_sess = str(getattr(self, "_last_session", "") or "")
                sess_now = last_sess or et_session_now()
                alarm = load_alarm()
                if alarm.wake_at and not session_parks(sess_now):
                    # Leftover RTH sit clock is not a park. Wipe it.
                    try:
                        clear_park()
                    except Exception:
                        pass
                ...
                future_park = (
                    bool(alarm.wake_at)
                    and not alarm.due()
                    and session_parks(sess_now)
                )
                if not poked and not resume and future_park:
                    first_think = False
                    if alarm.due():
                        self._resume_think = True
                        continue
                ...
                if (
                    ...
                    and not resume
                    and not first_think
                    and not paper_stay_up(session=sess_now)
                ):
                    # Live RTH after a look: sit until poke / Start. Not a sit clock.
                    self.state.status = "On"
                    await asyncio.sleep(0.25)
                    continue
                ...
                if not _wake_grok_for_session(session, needs_prot=needs_prot):
                    try:
                        ...
                        self._note("PARK", "overnight park seed failed")
                    self.state.status = "Waiting"
                ...
                try:
                    ...
                    if out.get("_parked") and session_parks(session):
                    ...
                    self.state.status = "On"
                    self.ui.put(("cycle", out))
                    # Clerk is not a runner. Finished RTH looks write no sit clock.
                    if not out.get("_failed"):
                        try:
                            if session_parks(session):
                                ensure_park(
                                    flat=flat_book,
                                    session=session,
                                    minutes_to_open=mins_open,
                            ...
                            self._note("PARK", "park/clear failed")
